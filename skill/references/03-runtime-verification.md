...
`www` の下に DKIM の鍵は置かないので、URL のホスト名で引くと「無い」と誤る。
**DNS が応答しないときは「無い」と判定しない。** タイムアウトの文言は値ではない。`recon.sh` は
「取得できない」と出し、その項目の判定を出さない。
...
ヘッダは**転送を追った最終の応答**で判定し、`Set-Cookie` は値を伏せて名前と属性だけを出す。
**最初の取得で到達できなければ、HTTP の節は判定を出さずに飛ばす**（「無い」という意味ではない）。
