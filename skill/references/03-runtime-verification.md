...
結果: 25 rows。`rowsecurity = false` は 0 件。
...
集約や整形をした場合は、**元の行数と、何をしたか**を添える。「162 行返ったが読みづらいので、テーブル×ロールで集約した結果を以下に示す（集約後 50 行、権限の総数は一致）」のように書く。
...
|---|---|---|---|---|
| アクセス制御 | 全テーブルの行レベル権限 | 25 テーブルすべて有効。無効は 0 件 | 問題なし | S-02 |
| 認証 | 多要素認証の登録状況 | 登録済み要素 0 件（利用者 9 名全員が未登録） | 問題あり | S-08 |
| 事業継続 | バックアップ | 無償プランのため機能自体が無い | 問題あり | S-20 |
