...
"""指摘台帳（xlsx）の雛形を生成する。
    ...
    python make_register.py <出力先.xlsx> [--service "サービス名"] [--date 2026-08-12]
...
def sheet_runtime(wb, names):
    ...
    note(ws, 7,
         ...
         "元の行数と何をしたかを添える。例:「162 行返ったが読みづらいため、テーブル×ロールで集約"
         "（集約後 50 行、権限の総数は一致）」。", 5)
    return ws
...
def sheet_findings(wb, names):
    ...
    body_row(ws, 4, [
        "S-01", "P1", "濫用対策 / メール",
        "/api/report が認証・レート制限・BOT 対策のいずれも通さず、リクエストボディの email に指定した"
        "任意のアドレスへ、自社ドメイン差出人の PDF 添付メールを送信する。アドレスの形式チェックも無い。",
        "app/api/report/route.ts:42-58",
        "第三者が自社ドメインを騙ってフィッシングメールを送れる。送信枠の枯渇とドメイン評判の毀損により、"
        "正規の通知メールが届かなくなる。",
        1.5, 1,
        "送信先を、同一リクエスト内で検証されたアドレスに限定する。既存の /api/lead にハニーポットと"
        "滞在時間判定が実装済みなので移植する。レート制限はエッジ側のルールで掛ける。",
        None,
    ], example=True)
...
def sheet_roadmap(wb, names):
    ...
    body_row(ws, 5, ["フェーズ1", "1〜2週間", "S-01", "/api/report の無認証メール送信を塞ぐ", 1.5, 1,
                     "検証時に運営宛て通知が実際に飛ぶ点に注意。実行前に周知する"], example=True)
    return ws
